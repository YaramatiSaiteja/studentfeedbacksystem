...
import { getCourses, getAllFeedback } from '../utils/api';
import { getCurrentUser, logoutUser } from '../utils/auth';
import { useTheme } from '../context/ThemeContext';
import { FaHome, FaChartLine, FaUsers, FaSun, FaMoon, FaBars, FaGraduationCap, FaUserCircle, FaBell, FaSignOutAlt, FaBookOpen, FaDownload, FaStar, FaChartPie, FaSearch } from 'react-icons/fa';
...
const AdminReports = () => {
    ...
    const loadReportData = async () => {
        try {
            const [courses, feedback] = await Promise.all([getCourses(), getAllFeedback()]);
            ...
            const visibleFeedback = currentDepartment
                ? (feedback || []).filter(item => visibleCourseIds.has(String(item.courseId)))
                : (feedback || []);

            // Aggregate feedback per course
            const aggregated = visibleCourses.map(course => {
                const courseFeedback = visibleFeedback.filter(f => String(f.courseId) === String(course.id));
    ...
    const filteredData = reportData.filter((item) => {
        ...
        return [
            ...
            item.instructor,
            item.totalFeedback,
            item.avgCourseRating,
            item.avgInstructorRating,
            item.avgContentRating,
            item.avgPracticalRating
                                        ...
                                        sortedData.map((data) => (
                                            <tr key={data.id}>
                                                <td className="ps-4 py-3">
                                                    ...
                                                    <div className="small text-muted">ID: {String(data.id).substring(0, 8)}</div>
                                                </td>
                                                <td>
