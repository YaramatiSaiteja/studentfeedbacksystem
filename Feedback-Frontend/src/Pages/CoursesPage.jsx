...
import { getCourses, getPendingCourses, getStudentFeedback } from '../utils/api';
...
const CoursesPage = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            if (user?.id) {
                try {
                    const [coursesData, pendingData, feedbackData] = await Promise.all([
                        getCourses(),
                    ...
                    const visibleCourses = department
                        ? (coursesData || []).filter(course => (course.department || course.branch) === department)
                        : (coursesData || []);
                    const visibleCourseIds = new Set(visibleCourses.map(course => String(course.id)));
                    setCourses(visibleCourses);
                    setPendingCourses((pendingData || []).filter(course => visibleCourseIds.has(String(course.id))));
                    setStudentFeedback((feedbackData || []).filter(item => visibleCourseIds.has(String(item.courseId))));
                } catch (err) {
                    ...
                    <ul className="navbar-nav ms-auto flex-row align-items-center gap-3">
                        ...
                        <li className="nav-item dropdown d-flex align-items-center gap-2">
                            <div className="d-none d-sm-flex flex-column text-end">
                                ...
                                <span className="text-muted lh-sm" style={{ fontSize: '0.78rem' }}>{user?.department || user?.role}</span>
                            </div>
                            <button
